...
The repository is now run with pnpm workspaces. There is no `turbo` layer.
...
```bash
pnpm cf:d1:create:stocks
pnpm cf:d1:create:crypto
```
...
- Backend Worker project
  ...
  - Deploy Command: `pnpm --dir apps/api deploy`
  - Production Branch: `main`
- Frontend Worker project
  - Root Directory: `/`
  - Build Command: `pnpm install --frozen-lockfile`
  - Deploy Command: `pnpm --dir apps/web deploy`
  - Production Branch: `main`
...
- If that legacy project still uses an old deploy command, migrate it to `pnpm deploy`.
