...
```bash
bun run dev:web      # start the web app in vinext dev mode
bun run check:web    # run vinext type checks and TypeScript
bun run test         # run the shared Bun test suite in /tests
bun run deploy:web   # build apps/web and deploy with Wrangler
```

From `apps/web`, the equivalent commands are `bun run dev`, `bun run check`, `bun run build`, and `bun run deploy`.
...
## Testing Guidelines
Tests use Bun test runner (`bun test`), not Jest or Vitest. Add coverage in `tests/*.test.ts` and name cases with explicit behavior statements, for example `test("loadMarketPageData drives market page server fetches", ...)`. Favor small unit tests around loaders, parsers, and shared contracts.

## Commit & Pull Request Guidelines
Recent history favors short imperative subjects, usually Conventional Commit style: `feat:`, `fix:`, `refactor:`, and `chore:`. Keep each commit focused and mention the affected area when helpful, for example `feat: add localized crypto archive filters`. Pull requests should include a brief summary, linked issue or context, commands run (`bun run check:web`, `bun run test`), and screenshots for UI changes.
